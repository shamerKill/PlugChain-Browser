        ...
        setTableContent(data.Txs.map((tx: any) => {
          ...
          return {
            ...
            value: [
              ...
              { key: getOnlyId(), value: <ComConLink link={`https://www.plugchain.network/v2/transDetail?id=${tx.hash}`}>{ tx.hash }</ComConLink> },
              { key: getOnlyId(), value: <ComConLink link={`https://www.plugchain.network/v2/blockDetail?id=${tx.block_id}`}>{ tx.block_id }</ComConLink> },
              { key: getOnlyId(), value: formatTime(tx.create_time) },
              { key: getOnlyId(), value: <ComConLink noLink={txTypeOutput} link={`https://www.plugchain.network/v2${walletVerifyAddress(tx.from)}`}>{ tx.from }</ComConLink> },
              { key: getOnlyId(), value: <ComConLink noLink={!txTypeOutput} link={`https://www.plugchain.network/v2${walletVerifyAddress(tx.to)}`}>{ tx.to }</ComConLink> },
